...
class OneBitCompletion:
    def __init__(self, arm_set, Theta_star):
        ...
        self.K = len(arm_set)
        self.X_arms = np.ascontiguousarray(np.concatenate([arm.flatten().reshape(1,-1) for arm in arm_set], axis=0), dtype=np.float64)
...
def E_optimal_design(env):
    """
    E-optimal design for the GL-LowPopArt algorithm.
    """
    X_arms = env.X_arms
    K = env.K
...
def GL_LowPopArt(env, N2, d1, d2, delta, Theta0, X1, y1, c_nu=1):
    ...
    # nu = np.sqrt(np.log(4*(d1 + d2) / delta) / (design_value * N2)) * 1e7
    nu = np.sqrt(np.log(4*(d1 + d2) / delta) / (design_value * N2)) * c_nu
    # print("nu:", nu)

    Theta_Catoni = np.zeros((d1, d2))
    for t, y in enumerate(y2):
        x = X2[t].flatten().reshape((d_, 1))

        # one-sample estimator
        vector_one_sample = (y - sigmoid(np.sum(theta0 * x))) * H_inv_optimal @ x
        matrix_one_sample = np.reshape(vector_one_sample, (d1, d2))
    ...
    # S_truncated = S
    # S_truncated[3:] = 0
    tau = np.sqrt(design_value * np.log(4*(d1 + d2) / delta) / (c_nu * N2))
