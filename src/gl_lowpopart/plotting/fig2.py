"""Plot Figure 2 from JSON outputs."""
...
def main():
    parser = argparse.ArgumentParser(description="Plot Figure 2 results")
    parser.add_argument("--model", type=str, choices=["bernoulli", "poisson"], default="bernoulli")
    args = parser.parse_args()

    completion_data, recovery_data = load_pair("Fig2", args.model)
    completion_Ns = completion_data["metadata"]["Ns"]
    recovery_Ns = recovery_data["metadata"]["Ns"]

    methods = {"uniform": "U+GL", "e_optimal": "E+GL", "zero": "0+GL", "random": "Rand+GL"}
    styles = {
        "e_optimal": {"color": "#0072B2", "linestyle": "-", "marker": "^"},
        "uniform": {"color": "#0072B2", "linestyle": "--", "marker": "s"},
        "zero": {"color": "#E69F00", "linestyle": "-", "marker": "o"},
        "random": {"color": "#009E73", "linestyle": "-", "marker": "D"},
    }
    ...
    plt.tight_layout()
    save_outputs("fig2", args.model)

    suffix = "" if args.model == "bernoulli" else f"_{args.model}"
    with open(f"{RESULTS_JSON_DIR}/fig2_results{suffix}.json", "w") as f:
        json.dump(
            {
